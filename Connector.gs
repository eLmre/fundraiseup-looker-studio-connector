...
let schema = [
    ...
    {name: 'device_type_mobile', label: 'Device Type Mobile', description: '', group: '', dataType: 'NUMBER', formula: 'CASE WHEN device_type = "mobile" THEN 1 ELSE 0 END '},
];
...
function getConfig(request) {
    ...
    /**
     * FundraiseUp REST Api does not support timestamp queries (as of today).
...
function fetchDataApi(request, skip_cache = false) {
    ...
    if( request && 'configParams' in request ) {
    ...
    let url = 'https://api.fundraiseup.com/v1/donations?limit=100'; // max 100
    let starting_after = ('data' in data && data.data.length ) ? '&starting_after='+ data.data[data.data.length - 1].id : '';
    let request_count = 0;
    ...
    do {
        ...
        */
        ...
        console.log('fetchDataApi() - Page: '+ request_count +' URL:', url + starting_after);

        let response = UrlFetchApp.fetch(url + starting_after, { headers: headers }); // throw error if not 200
        ...
        // ID of the last object in that list as the starting_after parameter in your subsequent request to retrieve the next page of results
        starting_after = '&starting_after='+ parsedResponse.data[parsedResponse.data.length - 1].id;
    ...
    Async.apply('fetchDataApi', ['', true], {after: 5 * 60 * 60 * 1000 });
...
function buildRows(request, data) {
...
function validateValue(field, value) {
    if( ['created_at', 'succeeded_at', 'failed_at', 'refunded_at', 'recurring_plan_ended_at'].includes(field.name) && value !== null ) {
        ...
        if( 'recurring_plan_ended_at' == field.name ) {
            console.log( 'recurring_plan_ended_at value before:', value );
        }
        ...
        if( 'recurring_plan_ended_at' == field.name ) {
            console.log( 'recurring_plan_ended_at value after:', value );
        }
